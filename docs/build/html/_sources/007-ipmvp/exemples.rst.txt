IPMVP — Example complet (Option C)
==================================
...
``table_inc`` — incertitude du model (niveau de confiance 80 %) :
...
``df_savings`` — économies (détaillées au chapter :doc:`mesure_economies`) :
...
The model de référence est ``Conso = 3564,64 × DJU − 320568,49`` : il explique
...
meilleure granularité). The calculation des économies est traité au chapter suivant,
:doc:`mesure_economies`.

.. note::
   ``Mathematical_Models`` **returns a 9-item tuple** ; il n'existe pas
   d'objet ``model`` with attributs ``.r2`` ou methods ``plot_*``. Les figures
   et les tables baseline **et** reporting sont produites in le rapport
   ``.docx`` généré by ``print_report=True`` (via ``docx_report``).
