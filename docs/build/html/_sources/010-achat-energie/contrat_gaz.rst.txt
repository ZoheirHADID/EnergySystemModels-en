...
**Example (T4, CJA = 109 MWh/j, ATRD7 2025-2026) :**
...
.. note::
   ...
   of the AHU (distribution et transport). C'est ce montant mensuel qui apparait sur
...
.. list-table::
   ...
   * - **TCL** (capacite de livraison)
     - ``CJN x TCL_PITD``
     - Cout for la livraison au point d'interface transport/distribution (PITD)
...
.. list-table::
   ...
   * - **coef_stockage**
     - Coefficient unitaire de stockage (euro/MWh/j), ex : 331,44 for 2025-2026
...
Ou ``Int`` est la somme des capacites interruptibles contractualisees (0 by defaut).
...
*TCR — Terme de Capacite Regionale (euro/MWh/j/an), pondere by NTR :*
...
Le prix unitaire est negocie in le contrat de fourniture.
...
.. list-table::
   ...
   * - ``P016``
     - Thermo-sensibilite standard (valeur by defaut in le modele)
   * - ``P017 a P018``
...
Ces valeurs sont publiees by la CRE et stockees in ``coefficients_gaz_ATRT.json``.
Le coefficient A for ATRT6 est fixe a 1.0 by defaut (valeurs exactes by GRD
disponibles in les deliberations annuelles ATRT6).

------------------------------------------------------------

4. Modele de calcul et example Python
------------------------------------------------------------
...
.. note::
   ...
   peuvent etre factures by GRDF separement according to le type de contrat.
...
.. list-table::
   ...
   * - type_tarif_acheminement
     - "T1", "T2", "T3", "T4", "TP"
     - Option tarifaire according to la CAR
   * - CAR_MWh
     - >= 0
