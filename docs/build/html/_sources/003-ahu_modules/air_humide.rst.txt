...
The module ``AHU.air_humide`` fournit les fonctions psychrométriques de l'air
humide, basées on l'ASHRAE Handbook — Fundamentals (Hyland & Wexler, 1983).
Each fonction accepte plusieurs combinaisons d'inputs (``T_db``, ``RH``,
...
Un example by fonction
-----------------------

Toutes les fonctions sont ici évaluées on le **même état d'air** :
``T_db = 25 °C``, ``RH = 50 %``, ``P = 101325 Pa``. Les valeurs en commentaire
``# ->`` sont les **real outputs are shown** de the library.
...
Les inputs sont interchangeables : for example ``Air_w`` accepte ``RH``,
``h`` ou ``T_wb`` en plus de ``T_db``, et ``Air_T_db`` reconstruit la
temperature from deux grandeurs quelconques (ici ``w`` et ``h``).

.. note::

   The function ``T_sat(w_target)`` (temperature de saturation for une humidité
...
* **Unités** : temperatures en °C, pressures en Pa, humidité absolue ``w`` en
  g/kg d'air sec, enthalpy ``h`` en kJ/kg d'air sec, masse volumique en kg/m³,
  volume spécifique en m³/kg.
* **Pressure** : by défaut ``P = 101325 Pa`` (niveau de la mer) ; ajuster le
  paramètre ``P`` for une autre altitude.
* **Domaine de validité** : formule Hyland & Wexler de ``Pv_sat`` valable de
  −100 °C à 200 °C, plus précise between −20 °C et 60 °C.
* **Références** : ASHRAE Handbook — Fundamentals (2013), ch. 1 Psychrometrics ;
  Hyland & Wexler (1983) ; Roland Stull (2011) for la variante de ``T_wb``.
