...
The module ``CEE`` estime le volume de certificats (kWh cumac) d'une opération
standardisée via un dispatcher unique, ``calcul_CEE(fiche, **params)``. This
page donne **un example Python exécutable by fiche**, rangé en trois secteurs :
**Industrie – Utilités**, **Bâtiment & enveloppe**, et **Transport**. Toutes les
valeurs affichées ci-dessous sont produites en exécutant actually la
library.
...
Le prix interne ``CEE.euro_MWhcumac`` vaut **5 €/MWh cumac** by défaut
(modifiable) ; c'est lui qui donne la colonne ``euro`` et les amounts indiqués
...
Vingt-cinq sheets couvrant les utilités industrielles : moteurs et variation de
vitesse, chaudières et fours, froid, air comprimé, waste heat, isolation et
mesurage.
...
Quatre sheets « bâtiment industriel » (IND-BA) et deux sheets « enveloppe outre-mer »
(IND-EN).
...
Deux sheets de transport intermodal.
...
Deux sheets restent in le registre for l'historique mais sont **exclues de**
``list_sheets()`` et refusées by ``calcul_CEE`` (``ValueError``) :
...
* utiliser les noms exacts des paramètres attendus by chaque fiche (une valeur
  d'énumération invalide lève une ``ValueError`` listant les valeurs acceptées) ;
* garder les unités du module (power en kW, surface en m², temperature en °C) ;
* ``return_details=True`` for tracer ``kWh_cumac`` / ``MWh_cumac`` / ``euro`` /
  ``titre`` in un audit ;
* ajuster ``CEE.euro_MWhcumac`` (défaut 5) ou appliquer un prix de marché externe ;
* vérifier l'éligibilité réglementaire on les sheets officielles (ADEME/ATEE)
  avant toute décision d'investissement.

Références
----------

* Opérations standardisées : https://www.ecologie.gouv.fr/operations-standardisees-deconomies-denergy
