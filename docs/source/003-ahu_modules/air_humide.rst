...
**Pour les temperatures supérieures ou égales à 0°C** (valable between 0°C et 200°C) :
...
- :math:`P_{v,sat}` = pressure de vapeur saturée (Pa)
- :math:`T_k` = temperature absolue (K) = °C + 273.15

**Constantes for T < 0°C :**
...
- :math:`P_v` = pressure partielle de vapeur d'eau (Pa)
- :math:`P` = pressure atmosphérique totale (Pa, by défaut 101325 Pa)
...
- :math:`h` = enthalpy (kJ/kg d'air sec)
- :math:`T_{db}` = temperature de bulbe sec (°C)
- :math:`w` = humidité absolue (g/kg d'air sec)
...
- :math:`R_a = 287.66` J/(kg·K) = constante des gaz for l'air sec
- :math:`R_v = 461` J/(kg·K) = constante des gaz for la vapeur d'eau
...
**Retour :** Pressure de vapeur saturée (Pa)
...
- ``Pv_sat`` : Pressure de vapeur saturée (Pa) - optionnel
- ``T_db`` : Temperature de bulbe sec (°C)
- ``h`` : Enthalpy (kJ/kg)
...
- ``w`` : Humidité absolue (g/kg)
- ``h`` : Enthalpy (kJ/kg)
- ``Pv_sat`` : Pressure de vapeur saturée (Pa)
- ``P`` : Pressure atmosphérique (Pa) - défaut : 101325 Pa

**Retour :** Temperature de bulbe sec (°C)
...
**Retour :** Temperature de bulbe humide (°C)
...
**Retour :** Temperature de saturation (°C)
...
Examples d'application complète
--------------------------------

Example 1 : Analyse d'une centrale de traitement d'air (CTA)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
Example 2 : Refroidissement évaporatif
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
Example 3 : Mélange de deux flux d'air
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
Example 4 : Dimensionnement d'une batterie froide
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
1. **Pressure atmosphérique** : Par défaut, tous les calculs utilisent P = 101325 Pa (niveau de la mer). Pour des altitudes différentes, ajuster le paramètre ``P``.

2. **Domaines de validité** :
   
   - Formules Hyland-Wexler for Pv_sat : -100°C à 200°C
   - Les calculs sont plus précis in la plage -20°C à 60°C

3. **Convergence numérique** : Certaines fonctions utilisent des methods itératives (fsolve). En cas de non-convergence, vérifier la cohérence physique des paramètres d'input.
...
5. **Références** :
   ...
   - Roland Stull (2011) for la method alternative T_wb
