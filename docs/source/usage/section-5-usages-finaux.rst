...
The module IPMVP mesure et vérifie les energy savings (Option C). La
fonction ``Mathematical_Models`` ajuste un model de baseline then quantifie les
économies ; elle **returns a 9-item tuple** (il n'existe no class
...
Output realle (extrait) : ``r2`` = 0,81, économie **ANTE-POST = 18,15 %**.

Chapitre détaillé : example complet, mesure des économies, incertitude propagée
(``incertitude_savings``), paramètres et théorie in :doc:`../007-ipmvp/index`.
