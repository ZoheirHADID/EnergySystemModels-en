...
* Rendement LHV et HHV by method indirecte
* Losss : fumées (sensible), CO, radiation, condensation
* Port eau (``Inlet``/``Outlet``) compatible FluidPort — consigne de temperature
...
.. list-table::
   ...
   * - gas_composition
     ...
     - (obligatoire)
   * - ng_flow_Nm3h
     - Flow rate gaz naturel
     - Nm3/h
     - None (auto si To défini)
   * - flue_gas_temperature_C
     - Temperature fumées output chaudière
     - °C
     - 180
   * - O2_measured
     - O2 mesuré en output
   ...
   * - design_useful_heat_kW
     - Power utile nominale
     - kW
     - 1000

Example 1 : Rendement à flow rate imposé
----------------------------------------
...
Le flow rate GN est calculé automatiquement à partir du Qth demandé et de l'efficacité.

Example 3 : Port fumées et dimensionnement économiseur
-------------------------------------------------------

Le port ``FG_Outlet`` allows dimensionner un économiseur en aval :
...
.. list-table::
   ...
   * - ``condensation_heat(T_C)``
     - Chaleur récupérable by condensation
     - (kW/(Nm3/h), kg/(Nm3/h))
