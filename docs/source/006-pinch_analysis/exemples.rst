...
.. figure:: ../images/006_pinch_distillation.svg
   ...
   Les flux de condenseur, rebouilleur et procédé sont réunis in la même
   analyse for identifier les échanges récupérables.
...
2. **Utilité chaude** : Steam heats the Reboiler → 300 kW requis (instead of 360 kW without integration)
...
* Le plot prévu est ``pinch.plot_GCC()`` enrichi by les lignes horizontales
...
* le graphique Matplotlib compare l'évolution des utilités with ``ΔTmin``.
