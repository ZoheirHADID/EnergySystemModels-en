...
.. figure:: ../images/006_pinch_base.svg
   :alt: Diagram d'une analyse Pinch de base
   :align: center

   Deux flux chauds à refroidir et deux flux froids à chauffer. L'analyse Pinch
   détermine combien de chaleur ces flux peuvent s'échanger between eux, et donc
   les utilités (vapeur, eau de refroidissement) actually nécessaires.
...
* ``STi`` / ``STo`` : temperatures **décalées** de ``±dTmin2`` (−5 K for les
  chauds, +5 K for les froids) — l'astuce qui garantit un écart real de
  ``ΔTmin`` between tout flux chaud et tout flux froid.
* ``delta_H = mCp × (To − Ti)`` : enthalpy échangée by le flux.
...
.. figure:: ../images/006_pinch_plot_composites.svg
   :alt: Courbes composites chaude et froide (output realle)
   :align: center

   Composite chaude (bleu) et composite froide (orange) en temperatures
...
.. figure:: ../images/006_pinch_plot_gcc.svg
   :alt: Grande courbe composite (output realle)
   :align: center
...
.. figure:: ../images/006_pinch_plot_hen.svg
   :alt: Réseau d'échangeurs de chaleur (output realle)
   :align: center
...
s'annule (temperature décalée 55 °C, soit **60 °C côté chaud** et **50 °C côté
froid**). Il coupe le procédé en deux zones indépendantes et impose trois règles
d'or, respectées by le network ci-dessus :
