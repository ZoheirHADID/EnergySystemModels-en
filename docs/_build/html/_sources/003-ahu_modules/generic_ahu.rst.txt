...
.. note::
   Il n'existe **pas** de classe ``GenericAHU`` ni de methods ``create_template``
   / ``run_simulation`` : on instancie directement ``AirRecyclingAHU`` ou
   ``AirRecoveryAHU`` with un ``config`` (dict) et un ``data`` (DataFrame). Un
   utilitaire ``test/AHU/utility_create_template_GenericAHU.py`` allows générer
...
**Data d'input** (colonnes du ``DataFrame``) : ``Timestamp``,
...
post-chauffage 11,41 kW for atteindre la consigne de soufflage 18 °C
(``POSTHC_Outlet_T``).

Mode récupération — ``AirRecoveryAHU``
--------------------------------------

Même principe, mais l'air neuf est préchauffé by un échangeur on l'air extrait.
La ``config`` remplace ``recycling`` by ``exchange_type`` / ``heat_exchanger`` et
``heat_exchanger_efficiency`` ; les data ajoutent ``Fresh Air [m3/h]``,
...
``ahu.df`` regroupe, for chaque time step, l'état psychrométrique à la
output de chaque composant, préfixé by un code de station :
